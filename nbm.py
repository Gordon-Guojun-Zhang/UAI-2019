...
# Bernoulli distribution, return y^{1-x}*(1-y)^x
def bern(x, y):
    return y if x else 1 - y
...
def project(theta):   
    ''' project theta onto the simplex, see eg. https://web.stanford.edu/~jduchi/projects/DuchiShSiCh08.pdf, use the simplest one for now'''
    m = len(theta)
...
def eff_mean(n, theta, Phi, s_size, S, cluster):
    ...
    for t in S:
        ...
        P_omega = np.dot(theta, Bern)  # regularize
        x_c += np.multiply((ct / P_omega) * Bern[cluster] * theta[cluster], x)
    N_c = eff_num(n, theta, Phi, s_size, S, cluster)
    x_c = np.divide(x_c, N_c)
    return x_c

def gradient(m, n, theta, Phi, s_size, S):
    ...
    for t in S:
        ...
        # theta_inc -= (ct / P_omega) * Bern
        Phi_inc = ct * np.divide(Phi_inc, P_omega)
        grad -= Phi_inc
    for c in range(m):
        theta_inc[c] = eff_num_pur(n, theta, Phi, s_size, S, c)
    return [- theta_inc / s_size, grad / s_size]
...
def gradientDescent(n, sample_size, S, theta, Phi, numIterations, alpha, tolerance=1e-6):
    ...
    for i in range(numIterations):
        ...
        oldPhi = np.copy(Phi)
        [theta_inc, grad] = gradient(m, n, theta, Phi, sample_size, S)
        ...
        for c in range(m):
            for j in range(n):
                ''' impose soft constraint on Phi'''
                Phi[c, j] = max(Phi[c, j], 1e-7)
                Phi[c, j] = min(Phi[c, j], 1-1e-7)
        ll = 0.0
        for t in S:
            ct = S[t]
            x = decToBits(t, n)
            ll -= ct * np.log(nbm(n, Phi, theta, x))
        #print("new loss: ", ll / sample_size)
        diff1 = np.linalg.norm(oldtheta - theta) / alpha
        if diff1 < tolerance and np.linalg.norm(grad) < tolerance:
 #           print("GD converges at ", i, "steps")
            break
    ll /= sample_size
    return [ll, theta, Phi, iterId]

def sgd(n, sample_size, l, S, theta, Phi, numIterations, alpha, tolerance=1e-5):
    return 0
...
def em(n, m, samples, S, theta, Phi, numIterations=20000, tolerance=1e-5):
    ...
    for _ in range(numIterations):
        ...
        if np.linalg.norm(oldtheta - theta1) < tolerance and np.linalg.norm(oldPhi - Phi1) < tolerance:
            break
#        print( "{:.{}f}".format(theta1[0], 4) )          
#        print("Phi: ", Phi1[0])
        #print("loss: ", ell(n, Phi1, theta1, S, samples))
...
def two_ell(m):     # compute m * log(m)
    total = 0.0
    for row in m:
        for elem in row:
            total += -np.log(elem) * elem if elem != 0 else 0
    return total
